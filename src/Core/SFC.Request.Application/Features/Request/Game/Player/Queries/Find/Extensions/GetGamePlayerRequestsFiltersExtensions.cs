...
using SFC.Request.Application.Features.Common.Constants;
...
public static class GetGamePlayerRequestsFiltersExtensions
{
    public static IEnumerable<Filter<GamePlayerRequest>> BuildSearchFilters(this GetGamePlayerRequestsFilterDto filter, DateTime now)
    {
        return [
            // Game
            new()
            {
                Condition = true,
                Expression = request => request.GameId == filter!.GameId
            },
            // Request
            new()
            {
    ...
    private static Expression<Func<GamePlayerRequest, bool>> FilterByStat(int? skill, short? from, short? to)
    {
        if (from.HasValue)
        {
    ...
    private static Expression<Func<GamePlayerRequest, bool>> FilterByRaiting(int? raiting)
    {
